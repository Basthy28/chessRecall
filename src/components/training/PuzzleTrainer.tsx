...
import {
  ...
  PuzzleRouteList,
} from "@/components/training/puzzle/puzzlePanels";
...
export default function PuzzleTrainer({ onOpenGameReview }: PuzzleTrainerProps) {
  ...
  const focusCards = [
    ...
    { label: "Best Move Gap", value: onlyMoveGap !== null ? `${(onlyMoveGap / 100).toFixed(2)}` : "Multi-choice" },
    ...
    >
      ...
      >
        ...
        <PuzzleHeaderBar
          ...
          phaseText={phaseLabel(currentPuzzle.phase)}
          ...
          isWrong={isWrong}
          isCorrect={isCorrect}
          showPlayedMove={showPlayedMove}
          playedInGameLabel={playedInGameLabel}
          onTogglePlayedMove={() => setShowPlayedMove((prev) => !prev)}
          onOpenAssociatedReview={currentPuzzle.game_id && onOpenGameReview ? () => onOpenGameReview(currentReviewTarget) : undefined}
        />
      ...
      >
        <PuzzleProgressSection
          ...
          sessionSolvedCount={sessionSolvedCount}
          sessionProgressPercent={sessionProgressPercent}
          routeSummary={routeSummary}
          currentStage={currentStage}
          phaseTitle={`${phaseLabel(currentPuzzle.phase)} ${currentPuzzle.player_color === "white" ? "attack" : "defense"}`}
